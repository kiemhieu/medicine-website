...
{
    public class MedicineDeliveryDetailClientAdapter : ClientAdapterBase
    {
        public MedicineDeliveryDetailClientAdapter(SqlConnection connection) : base(connection)
        {
        }

        protected override SqlCommand CreateSelectCommand(SqlConnection connection)
        {
            String commandBuilder = String.Format("Select * from MedicineDeliveryDetail Where LastSync IS NULL OR LastSync < LastUpdatedDate");
        ...
        protected override SqlCommand CreateUpdateCommand(SqlConnection connection)
        {
            ...
            commandBuilder.Append(" UPDATE MedicineDeliveryDetail SET LastSync = @lastSync WHERE Id = @id ");
